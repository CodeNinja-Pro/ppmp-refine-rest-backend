...
// export const PurchaseCartItemEdit: React.FC<IResourceComponentsProps> = () => {
...
import { Create, useDataGrid, useAutocomplete, Edit } from "@refinedev/mui";
import {
  Autocomplete,
  Box,
  Grid,
...
export const PurchaseCartItemEdit: React.FC<
...
> = ({ purchaseCartItems = [], setPurchaseCartItems }) => {
  ...
  React.useEffect(() => {
    const currentPurchaseCartItem = purchaseCartItems[(+id)-1|| 0] || {};
    setValue("product", currentPurchaseCartItem?.product);
    setValue("qty", currentPurchaseCartItem?.qty);
    setValue("supply_type", currentPurchaseCartItem?.supply_type);
    setValue("source_of_funding", currentPurchaseCartItem?.source_of_funding);
    setValue("method", currentPurchaseCartItem?.method)
    setValue("purchase_mode", currentPurchaseCartItem?.purchase_mode)


  }, [id, purchaseCartItems])
  React.useEffect(() => {
...
// console.log(errors);
  return (
    <Edit isLoading={formLoading} saveButtonProps={saveButtonProps} canDelete={false}>
      <Box
        component="form"
        ...
        <Grid container columnSpacing={5}>
          ...
          <Grid xs={6} md={3} item>
            <TextField
              ...
              minRows={10}
              disabled
              label={translate("purchase_cart_items.fields.total_amount")}
            />
          </Grid>
        </Grid>
      </Box>
